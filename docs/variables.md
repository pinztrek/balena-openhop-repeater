...
* **REGIONS** Set to `1` or `true` to load region/transport-key data — seeds `regions.yaml` from the default example into `/etc/openhop_repeater/` if not already present, then loads it into `repeater.db`'s `transport_keys` table (full replace) every restart, prior to starting _openhop_repeater_. On exit, the current DB state is dumped back out to `regions.yaml`, so any regions added/edited live (GUI/API) while running survive the next restart. `regions.yaml` is a plain YAML list (`name`, `flood_policy`, `transport_key`, optional `parent_name`) and is user-editable; see `dump-regions`/`load-regions` to manage it. Leave blank/unset to do nothing. **Note:** the bundled default `regions.yaml.example` reflects this repo maintainer's own regions and is very likely wrong for your install — tune it before building (edit `regions.yaml.example`) or after deploying (edit the seeded `regions.yaml` on the device).
